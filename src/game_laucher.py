...
class GameObject:
    def __init__(self, image_path, world_x, world_y, width_tiles=1, height_tiles=1, collision=False):
        try:
            ...
            self.image = pygame.Surface((width_tiles * TILE_SIZE, height_tiles * TILE_SIZE))
            self.image.fill((139, 69, 19))  # Marrom para árvores
            self.world_x = world_x
            self.world_y = world_y
            self.width_tiles = width_tiles
            self.height_tiles = height_tiles
    ...
    def load_objects(self):
        ...
        try:
            ...
            if self.level == 1:
                ...
                # Nível 2 - alguns objetos (só testando aqui)
                objects_data = [
                ...
                # Nível 3 - alguns objetos (só testando aqui)
                objects_data = [
...
class Player(Entity):
    ...
    def check_collision(self, new_x, new_y, tile_manager):
        """Verifica colisão com tiles e objetos - SIMPLIFICADO PARA TESTE"""
        # Criar rect do player na nova posição
        ...
        for obj in tile_manager.objects:
            if obj.collision:
                obj_rect = pygame.Rect(obj.world_x, obj.world_y, 
                                     obj.width_tiles * TILE_SIZE, 
                                     obj.height_tiles * TILE_SIZE)
                if player_rect.colliderect(obj_rect):
                    print(f"Colisão com objeto em ({obj.world_x}, {obj.world_y})")
                    return True
...
class Game:
    ...
    def run(self):
        ...
        while running:
            ...
            # 4. Objetos (árvores, etc)
            self.tile_manager.draw_objects(self.screen, self.camera_x, self.camera_y)
        ...
        # Retornar resultado
        success = self.level_completed and self.score >= 140  # 70% de 200
        print(f"\n=== RESULTADO ===")
